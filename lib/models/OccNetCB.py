...
class PoseResNet(nn.Module):
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels):
        ...
        for i in range(num_layers):
            ...
            for _ in range(2 if i == num_layers-1 or i == num_layers-2 else 1):
                layers.append(
                    nn.Sequential(
                        nn.ConvTranspose2d(
    ...
    def forward(self, x):
        ...
        x1 = self.deconv_layers[1](x)
        x2 = self.deconv_layers[2](x)
        x = x1 + x2
        x1 = self.deconv_layers[3](x)
        x2 = self.deconv_layers[4](x)
    ...
    def init_weights(self, pretrained=''):
        if os.path.isfile(pretrained):
            ...
            for m in self.final_layer1.modules():
